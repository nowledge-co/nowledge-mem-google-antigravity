...
def main():
    ...
    if not artifact_id and title:
        ...
        if existing:
            ...
            print(f"Notice: Found existing artifact '{existing.get('title')}' ({artifact_id}). Auto-binding for in-place update.")

    if not artifact_id:
        sys.stderr.write("Error: Could not determine target artifact ID. Specify --artifact-id or include 'id: <artifact_id>' in frontmatter.\n")
        sys.exit(1)
