...
* **Intent-Based Gating & Command Auto-Allowing**: Preserve the auto-allow logic in `hooks/nmem-gate.py`.
  ...
  - Under `PreToolUse` in `hooks.json`, the `run_command` tool is monitored. Auto-approve specific commands (such as the native status script `nmem_status.py`, `nmem status`, and `nmem tasks`) to bypass command confirmation prompts and provide a premium, prompt-free UX.
