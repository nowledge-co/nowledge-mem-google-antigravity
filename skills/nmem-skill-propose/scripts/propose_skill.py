...
def activate_skill(config, skill_id):
    if not skill_id or skill_id == "unknown":
        return False
    try:
        make_request(config, f"/skills/{skill_id}/activate", payload={}, method="POST")
...
def main():
    ...
    parser.add_argument(
        "--create-new", action="store_true", help="Force creation of a new skill even if a name match exists"
    )
    ...
    parser.add_argument(
        "--activate",
        "-a",
        dest="should_activate",
        action="store_true",
        default=True,
        help="Activate skill on Nowledge Mem after proposal/import or update (default)",
    )
    parser.add_argument(
        "--no-activate",
        dest="should_activate",
        action="store_false",
        help="Keep skill in proposal stage on Nowledge Mem without activating",
    )
    ...
    if not skill_id and not args.create_new and skill_name:
        ...
        if existing:
            ...
            print(
                f"Notice: Found existing skill '{existing.get('title') or skill_name}' ({skill_id}) [stage: {existing.get('stage')}]."
            )
            print(
                "Auto-binding request to perform in-place update. (Pass --create-new to force creation of a new skill)."
            )
    ...
    if skill_id:
        ...
        try:
            edit_res = make_request(config, "/agent/skill-builder/edit-body", {"skill_id": skill_id, "body": skill_md})
        ...
        try:
            response_data = make_request(
                config, "/agent/skill-builder/import", {"skill_md": skill_md, "force": args.force}
            )
            ...
            if not created and matched and matched.get("id"):
                ...
                make_request(config, "/agent/skill-builder/edit-body", {"skill_id": matched_id, "body": skill_md})
                if not args.no_apply:
